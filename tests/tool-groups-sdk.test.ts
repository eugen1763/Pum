...
describe("canonical tools through the installed SDK request and prompt construction", () => {
  ...
  test("main and child registration load trusted state before creating services", () => {
    ...
    const factory = main.indexOf("async ({ cwd, sessionManager, sessionStartEvent, conversationState, planMode }) => {");
    const construct = main.indexOf('new ToolGroupsController("main", undefined, restricted)');
