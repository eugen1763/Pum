...
import { buildStashBatchPrompt, selectedRange } from "../src/stash-batch";
...
describe("stash batch selection", () => {
  ...
  test("asks the main agent to group, run, and close shared or worktree tasks", () => {
    ...
    expect(prompt).toContain("Use the shared project by default");
    expect(prompt).toContain("Set worktree to true only when isolated changes");
    ...
    expect(prompt).toContain("Use worktree remove to close a shared-directory subagent");
