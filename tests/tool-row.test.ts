...
import { interruptedToolCall, settledToolCall, startedToolCall } from "../src/tool-row";
import { settledUserBashCall } from "../src/user-bash";
...
describe("a command the user ran themselves", () => {
  test("keeps no note from a settle that happened while it ran", () => {
    // The agent's turn can end while a `!` command is still running. The row is
    // excluded from that sweep, but if one ever reached it the result still has
    // to own the row rather than leave it reading "interrupted".
    const swept = interruptedToolCall({
      id: "user-bash-1",
      name: "bash",
      args: ["sleep 30"],
      state: "running",
    ...
    expect(swept.detail).toBe("interrupted");

    const settled = { ...swept, ...settledUserBashCall({
      output: "done",
      exitCode: 0,
      cancelled: false,
      truncated: false,
