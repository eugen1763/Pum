...
/** Effective policy observations append at each request boundary, never rewrite
 * the system prefix. Actual tool authorization always reads live policy. */
export const checkModePromptExtension: InlineExtension = {
  name: "pum-check-mode-rules",
  factory(pi) {
    registerOperationalNotice(pi, {
      customType: "pum.check_policy",
      observe: () => {
        const config = getCheckModeConfig();
        return buildCheckModePrompt({
