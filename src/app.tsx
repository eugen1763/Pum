...
export function App({
  ...
  // /settings is the one command whose arguments come from closed sets, so it
  // completes them. Every other command still stops at its name.
  const settingsSuggestions = shellMode || stashOpen || commandSuggestionsDismissed || activeAgentId
    ? []
  ...
  const runCommand = (text: string): boolean => {
    ...
    if (!compress && !clear && !historyCommand && !loginCommand && !checkPathCommand && !settingsCommand && !triggersCommand && !processesCommand && !newsCommand && !todoCommand && !statsCommand && !worktreeCommand) return false;
    setEditingStash(null);
  ...
  useKeyboard((key) => {
    ...
    const argumentMatches = settingsMatches.length > 0 ? settingsMatches : pathMatches;
    ...
    if (isPlainReturn && (settingsMatches.length > 0 || visiblePathMatches.length > 0)) {
      key.stopPropagation();
      const returnMatches = settingsMatches.length > 0 ? settingsMatches : visiblePathMatches;
      const index = Math.min(commandCursorRef.current, returnMatches.length - 1);
      const completed = applyPathCompletion(inputValue, returnMatches[index]!);
      setEditorText(completed.value, completed.cursorOffset, true);
      pathCompletionCycle.current = {
        sourceValue: inputValue,
        completions: returnMatches,
        index,
        currentValue: completed.value,
        currentCursor: completed.cursorOffset,
      };
        ...
        {suggestionCount > 0 ? (
          ...
          >
            {(commandSuggestions.length > 0
              ...
              ? settingsSuggestions.map((completion) => ({
